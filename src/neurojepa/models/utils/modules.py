...
from neurojepa.models.utils.moe import MoE, unpack_moe_params

from timm.layers import drop_path

# Note: torch.nn.attention.sdpa_kernel is intentionally NOT used here — its
# no-arg context-manager form triggers TorchDynamo graph breaks under compile.
    ...
    def forward(self, x, mask=None, attn_mask=None, T=None, H=None, W=None, action_tokens=0):
        ...
        if action_tokens > 0:
            ...
            for i in range(action_tokens):
                ...
                qkv = self.qkv(a).unflatten(-1, (3, self.num_heads, self.head_dim)).permute(2, 0, 3, 1, 4)
        ...
        qkv = self.qkv(x).unflatten(-1, (3, self.num_heads, self.head_dim)).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # [B, num_heads, N, D]
...
class RoPEAttention(nn.Module):
    def __init__(
        ...
        self.proj_drop_prob = proj_drop
        self.proj_drop = nn.Dropout(proj_drop)
        self.proj_attn_gate = nn.Linear(dim, num_heads, bias=qkv_bias)
        self.use_sdpa = use_sdpa
        # --
        self.d_dim = int(2 * ((head_dim // 3) // 2))
        self.h_dim = int(2 * ((head_dim // 3) // 2))
    ...
    def forward(self, x, mask=None, attn_mask=None, D_patches=None, H_patches=None, W_patches=None):
        B, N, C = x.size()
        grid_depth = int(N // (self.grid_size * self.grid_size))

        gate_score = self.proj_attn_gate(x).unsqueeze(-1)  # [B, N, num_heads, 1]
        qkv = self.qkv(x).unflatten(-1, (3, self.num_heads, self.head_dim)).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # [B, num_heads, N, D]
        ...
        if mask is not None:
            ...
            if D_patches is None or H_patches is None or W_patches is None:
                mask = torch.arange(int(grid_depth * self.grid_size * self.grid_size), device=x.device)
            else:
                mask = torch.arange(int(D_patches * H_patches * W_patches), device=x.device)
            d_mask, h_mask, w_mask = self.separate_positions(mask, H_patches, W_patches)
        ...
        # gating
        # exchange second and third dimension for x
        x = x.transpose(1, 2)  # [B, N, num_heads, D] (gate_score shape: [B, N, num_heads, 1])
        ...
        **kwargs,
        ...
        if act_layer is nn.SiLU:
            ...
            if not use_moe:
                ...
                (dim, n_shared_experts, n_routed_experts, 
                 n_activated_experts, moe_inter_dim, 
                 score_func, route_scale) = unpack_moe_params(moe_params)
                
                self.mlp = MoE(dim=dim, n_shared_experts=n_shared_experts, n_routed_experts=n_routed_experts,
                    n_activated_experts=n_activated_experts, moe_inter_dim=moe_inter_dim,
                    score_func=score_func, route_scale=route_scale)

    def forward(self, x, mask=None, attn_mask=None, D_patches=None, H_patches=None, W_patches=None, validity_mask=None):
        if isinstance(self.attn, RoPEAttention):
            y = self.attn(self.norm1(x), mask=mask, attn_mask=attn_mask, D_patches=D_patches, H_patches=H_patches, W_patches=W_patches)
        else:
            y = self.attn(self.norm1(x), mask=mask, attn_mask=attn_mask)
        x = x + self.drop_path(y)
        x_moe_scores = None
        if isinstance(self.mlp, MoE):
            x_mlp, x_moe_scores = self.mlp(self.norm2(x), mask=validity_mask)
        else:
            x_mlp = self.mlp(self.norm2(x))
        x = x + self.drop_path(x_mlp)
        
        return x, x_moe_scores
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, x, attn_mask=None):
        """
        :param q: query tokens [B, n, C]
        :param x: key/value tokens [B, N, C]
        :param attn_mask: optional boolean mask [B, N] or [B, 1, 1, N].
                          True = valid (attend), False = invalid (mask out).
        """
        B, n, C = q.shape
        ...
        # Prepare attn_mask for scaled_dot_product_attention: [B, 1, 1, N]
        sdpa_mask = None
        if attn_mask is not None:
            if attn_mask.dim() == 2:
                # [B, N] -> [B, 1, 1, N]
                sdpa_mask = attn_mask.unsqueeze(1).unsqueeze(1)
            else:
                sdpa_mask = attn_mask

        if self.use_sdpa:
            q = F.scaled_dot_product_attention(q, k, v, attn_mask=sdpa_mask)
        else:
            xattn = (q @ k.transpose(-2, -1)) * self.scale
            if sdpa_mask is not None:
                xattn = xattn.masked_fill(~sdpa_mask, float('-inf'))
