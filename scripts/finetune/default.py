...
def main(cfg):
    ...
    if cfg.wandb.wandb_enable and dist.get_rank() == 0:
        ...
        # Optional sweep grouping/tags (added by submit_sweep_jobs.py)
        if getattr(cfg.wandb, 'group', None):
            wandb_kwargs['group'] = cfg.wandb.group
        if getattr(cfg.wandb, 'tags', None):
            wandb_kwargs['tags'] = list(cfg.wandb.tags) if not isinstance(cfg.wandb.tags, list) else cfg.wandb.tags
    ...
    # count number of parameters for backbone
    num_backbone_params = sum(p.numel() for p in backbone.parameters())
    num_classifier_params = sum(p.numel() for p in classifier.parameters())
    logger.info(f"Number of parameters - Backbone: {num_backbone_params}, \
        Classifier: {num_classifier_params}")
    ...
    if cfg.meta.get("freeze_backbone", False):
    ...
    if cfg.data.num_classes != 1:
        # classification task
        from neurojepa.engines.finetune.clf import trainer
        criterion = nn.CrossEntropyLoss().to(device)
    else:
        # regression task
        from neurojepa.engines.finetune.reg import trainer
